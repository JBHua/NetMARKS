...
func Produce(w http.ResponseWriter, r *http.Request) {
	...
	for i := uint64(0); i < quantity; i++ {
		...
		go shared.ConcurrentHTTPRequest("http://"+CoalServiceAddr, "grain", &wg, ch)
		go shared.ConcurrentHTTPRequest("http://"+GoldServiceAddr, "water", &wg, ch)
		go func() {
			wg.Wait()
			close(ch)
		}()
		...
		for url, response := range responses {
			if response.Err != nil {
				...
				if response.Type == "gold" {
					...
					singleCoin.GoldId = water.Items[0].Id
				} else if response.Type == "coal" {
					...
					singleCoin.CoalId = grain.Items[0].Id
				}
			}
		}
