...
type IronoreServer struct {
	Ironore.UnimplementedIronoreServer
	breadClient Bread.BreadClient
	fishClient  Fish.FishClient
	meatClient  Meat.MeatClient
}

func NewIronoreServer(b Bread.BreadClient, f Fish.FishClient, m Meat.MeatClient) *IronoreServer {
	return &IronoreServer{
		breadClient: b,
		fishClient:  f,
		meatClient:  m,
	}
...
func newGRPCServer(lis net.Listener) error {
	...
	Ironore.RegisterIronoreServer(grpcServer, NewIronoreServer(breadClient, fishClient, meatClient))

	return grpcServer.Serve(lis)
}

func (s *IronoreServer) Produce(ctx context.Context, req *Ironore.Request) (*Ironore.Response, error) {
...
func Produce(w http.ResponseWriter, r *http.Request) {
	...
	for i := uint64(0); i < quantity; i++ {
		...
		go shared.ConcurrentHTTPRequest("http://"+BreadServiceAddr+"?response_size="+responseSize, "bread", NodeName, requestId, originalRequestService, &wg, ch)
		go shared.ConcurrentHTTPRequest("http://"+FishServiceAddr+"?response_size="+responseSize, "fish", NodeName, requestId, originalRequestService, &wg, ch)
		go shared.ConcurrentHTTPRequest("http://"+MeatServiceAddr+"?response_size="+responseSize, "meat", NodeName, requestId, originalRequestService, &wg, ch)
		go func() {
			wg.Wait()
			close(ch)
		}()
